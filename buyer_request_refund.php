...
function bv_mobile_create_refund_request(PDO $pdo, array $buyer, array $order, array $items, array $payload): array
{
    ...
    if (function_exists('bv_order_refund_create_request') || function_exists('bv_order_refund_create_from_cancellation')) {
        $helperPayload = [
            'buyer' => $buyer,
            'order' => $order,
            'items' => $items,
        ...
        foreach (['bv_order_refund_create_request', 'bv_order_refund_create_from_cancellation'] as $helper) {
            ...
            try {
                ...
                if ($reflection->getNumberOfRequiredParameters() <= 1) {
                    $created = $helper($helperPayload);
                    if (is_array($created) && !empty($created['id'])) {
                        return [
                            'id' => (int) $created['id'],
                            'refund_code' => (string) ($created['refund_code'] ?? $created['code'] ?? ''),
                            'status' => (string) ($created['status'] ?? 'pending_approval'),
                            'requested_refund_amount' => round((float) ($created['requested_refund_amount'] ?? $requestedAmount), 2),
                            'currency' => (string) ($created['currency'] ?? $currency),
                        ];
                    }
                }
            } catch (Throwable $e) {
                bv_mobile_log('Refund helper failed: ' . $e->getMessage());
            }
        }
    }
